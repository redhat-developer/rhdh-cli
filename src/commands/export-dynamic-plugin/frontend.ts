...
import {
  ...
  loadResolutionsFromYarnLockWorkspace,
} from './common-utils';
...
`
  ...
  const workspaceResolutions = await loadResolutionsFromYarnLockWorkspace({
    pluginPkgDir: paths.targetDir,
    monorepoRoot: paths.targetRoot,
  });
...
function buildFrontendYarnInstallCommand(
  yarn: string,
  yarnVersion: string,
  ...
  if (yarnVersion.startsWith('1.')) {
    const lockFlag = yarnLockExists ? ' --frozen-lockfile' : '';
    return `${yarn} install --production${lockFlag} ${redirect}`;
  }
  const lockFlag = yarnLockExists ? ' --immutable' : ' --no-immutable';
  return `${yarn} install${lockFlag} ${redirect}`;
}
...
async function handlePackageInstall(opts: OptionValues, target: string) {
  ...
  await initializeYarnProject({
    pluginPkgDir: paths.targetDir,
    monorepoRoot: paths.targetRoot,
    exportDir: target,
    yarnVersion,
    copyYarnLockIfMissing: !yarnLockExists,
  });
  ...
  const actionVerb = yarnLockExists ? 'Verifying' : 'Creating';
  Task.log(`${actionVerb} filtered yarn.lock file for the exported package`);

  const logFile = path.join(os.tmpdir(), 'rhdh-cli.yarn-install.log');
  const redirect = `> ${logFile}`;
  const yarnInstall = buildFrontendYarnInstallCommand(
    yarn,
    yarnVersion,
    yarnLockExists,
    redirect,
  ...
  await fs.remove(path.join(target, 'node_modules'));
  await fs.remove(path.join(target, '.yarn'));
